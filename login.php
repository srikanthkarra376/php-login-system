...
if (isset($_POST["login-btn"])) {
  ...
  if (count($login_errors) == 0) {
    ...
    $stmt = "SELECT * FROM users WHERE  username='$login_uname' or email ='$login_uname'";
    ...
    if ($result = mysqli_query($connection, $stmt)) {
      ...
      if (mysqli_num_rows($result) == 1) {
        ...
        $db_check_username_err = "Username does not exist !";
      }
    }
  }
}
...
<div class="container mt-4 col-md-4 bg-light">
    ...
    <small class="text-success">
        <?php if (isset($_GET["loginMsg"])) echo '*' . $_GET["loginMsg"] ?>
    </small>
    <h1 class="text-center text-muted">login </h1>

    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
        <div class="form-group">
            <input type="text" class="form-control" name="login-uname" placeholder="Username or Email" value="<?php if (isset($login_uname)) echo $login_uname; ?>">
